...
const SlotCard = ({ onSelectSlot }) => {
  ...
  useEffect(() => {
    ...
    // 💡 initialize after days are ready
    setDays(dynamicDays);
    setSelectedDay(dynamicDays[0]); // store the full object instead of just date

    // Generate 3 upcoming time slots (30-min intervals)
    const slots = [];
    ...
    while (slots.length < 6) {
      ...
      // Stop if we exceed service hours
      ...
      baseTime = addMinutes(baseTime, 60); // increment 1 hour
    }

    // 💡 store full object for easier selection later
    setTimeSlots(slots);
    setSelectedTime(slots[0]);
  }, []);

  // 💡 Now we already store full objects, no need to find again
  const handleProceed = () => {
    if (selectedDay && selectedTime) {
      onSelectSlot({ day: selectedDay, time: selectedTime });
    }
  };
